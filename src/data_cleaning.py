...
def calculate_ytd_fantasy_points(
    df: pd.DataFrame, game_week: int
) -> pd.DataFrame:
    ...
    return (
        df[df["rd"] <= game_week][["name", "id", "pts"]]
        .groupby(["id", "name"])
        .sum()
    )
...
def meta_feature_prep(df: pd.DataFrame) -> pd.DataFrame:
    ...
    features = pd.concat(
        [df[["id", "name", "salary"]], encode_categories(df[["position"]])],
        axis=1,
    )
...
def season_feature_target_prep(
    ...
    features.columns = [
        col.replace(" ", "_").lower() for col in features.columns
    ]
...
def merge_data(
    meta_data_filepath: str, top_data_filepath: str, season_data_filepath: str
) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def get_data_for_modeling(
    ...
    """
    features, targets = merge_data(
        meta_data_filepath, top_data_filepath, season_data_filepath
    )
